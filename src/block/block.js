 ...
 */
registerBlockType( 'cgb/block-microlink', {
	// Block name. Block names must be string that contains a namespace prefix. Example: my-plugin/my-custom-block.
	title: __( 'microlink' ), // Block title.
	...
	edit: ({ attributes, setAttributes, className })  => {
		...
		if ( ! self.fetch ) {
			//browser doesn't have fetch
		}

		const onChangeURL = async value => {
			//check if empty somewhere

			const response = await fetch( `https://api.microlink.io?url=${ value }`, {
				cache: 'no-cache',
				headers: {
					// this should be set to something useful
					'user-agent': 'WP Block',
